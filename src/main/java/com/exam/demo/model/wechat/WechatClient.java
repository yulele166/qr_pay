...
public class WechatClient {
     ...
     * @return
     */
    public WechatOrderQuery.Response orderQuery(String orderNo){
        WechatOrderQuery orderQuery = new WechatOrderQuery();
        orderQuery.setOut_trade_no(orderNo);
        return WechatConfig.getInstance().orderQuery(orderQuery);
    }
    /**
     * 微信订单查询
     * @param wechatOrderQuery 要查询的请求
     * @return 查询结果
     */
    public WechatOrderQuery.Response orderQuery(WechatOrderQuery wechatOrderQuery){

        wechatOrderQuery.setAppid(this.app_id);
        wechatOrderQuery.setMch_id(this.mch_id);
        wechatOrderQuery.setNonce_str(nonce_str(16));
        wechatOrderQuery.setSign(sign(SignUtil.bean2TreeMap(wechatOrderQuery)).toUpperCase());
        String tempXmlStr = XmlUtil.beanToXml(new ByteArrayOutputStream(), wechatOrderQuery);
        String requestXml = tempXmlStr!= null?tempXmlStr.substring(55):"";
        try {
            InputStreamEntity inputStreamEntity = new InputStreamEntity(
                    new ByteArrayInputStream(requestXml.getBytes("UTF-8")), ContentType.APPLICATION_XML);
